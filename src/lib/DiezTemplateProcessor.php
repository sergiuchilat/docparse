...
class DiezTemplateProcessor
{
    const MAXIMUM_REPLACEMENTS_DEFAULT = -1;

    private static $selectedTag = '';
    private static $tagOptions = null;
    ...
    protected static function ensureMacroCompleted($macro)
    {
        if (substr($macro, 0, 2) !== '${' && substr($macro, -1) !== '}') {
            $macro = self::$tagOptions[self::$selectedTag]['start'] . $macro . self::$tagOptions[self::$selectedTag]['end'];
        }
    ...
    {
        $matches = array();
        preg_match_all(self::$tagOptions[self::$selectedTag]['getVariablesForPart'], $documentPartXML, $matches);
        return $matches[1];
    }
    ...
    protected function indexClonedVariables($count, $xmlBlock)
    {
        $results = array();
        for ($i = 1; $i <= $count; $i++) {
            $results[] = preg_replace(self::$tagOptions[self::$selectedTag]['indexClonedVariables']['pattern'], self::$tagOptions[self::$selectedTag]['indexClonedVariables']['replacementStart'] . $i . self::$tagOptions[self::$selectedTag]['end'], $xmlBlock);
        }
    ...
    protected function splitTextIntoTexts($text)
    {
        ...
        $result = str_replace(array(self::$tagOptions[self::$selectedTag]['start'], self::$tagOptions[self::$selectedTag]['end']), array('</w:t></w:r><w:r>' . $extractedStyle . '<w:t xml:space="preserve">' . self::$tagOptions[self::$selectedTag]['start'], self::$tagOptions[self::$selectedTag]['end'] . '</w:t></w:r><w:r>' . $extractedStyle . '<w:t xml:space="preserve">'), $unformattedText);
    ...
    {
        return preg_match(self::$tagOptions[self::$selectedTag]['textNeedsSplitting'], $text) == 1;
    }
